...
def robot_human_vec_dot_product(robot_pos, human_pos, base_T):
    ...
    norm = np.linalg.norm(vector_human_robot)
    if norm < 1e-6:
        return 0.0
    vector_human_robot = vector_human_robot / norm
    forward_robot = np.array(base_T.transform_vector(mn.Vector3(1, 0, 0)))[
        [0, 2]
    ]
    forward_norm = np.linalg.norm(forward_robot)
    if forward_norm < 1e-6:
        return 0.0
    forward_robot = forward_robot / forward_norm
