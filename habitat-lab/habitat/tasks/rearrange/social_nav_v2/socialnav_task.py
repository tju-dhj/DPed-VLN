    ...
    def __init__(
        ...
        **kwargs,
        ...
        if hasattr(dataset,"config"):
            # Handle data_path formatting with scene parameter
            if "{scene}" in dataset.config.data_path:
                # For dynamic datasets, we need to provide a default scene or handle it differently
                data_path = dataset.config.data_path.format(split=dataset.config.split, scene="default")
            else:
