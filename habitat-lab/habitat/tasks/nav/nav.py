...
class SPL(Measure):
    ...
    def update_metric(
        ...
        # Avoid division by zero when both start_end and agent distances are 0
        denom = max(1e-6, self._start_end_episode_distance, self._agent_episode_distance)
        self._metric = ep_success * (self._start_end_episode_distance / denom)
...
class SoftSPL(SPL):
    ...
    def update_metric(self, episode, task, *args: Any, **kwargs: Any):
        ...
        # Guard against zero start distance
        start_dist = max(1e-6, self._start_end_episode_distance)
        ep_soft_success = max(0, (1 - distance_to_target / start_dist))
        ...
        denom = max(1e-6, self._start_end_episode_distance, self._agent_episode_distance)
        self._metric = ep_soft_success * (self._start_end_episode_distance / denom)
