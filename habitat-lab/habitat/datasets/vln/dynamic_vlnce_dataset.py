...
class DynamicVLNCEDatasetV1(PointNavDatasetV1):
    ...
    def _load_dataset_files(self, config: "DictConfig") -> None:
        ...
        if not self.episodes:
            raise ValueError(
                f"DynamicVLNCEDataset loaded 0 episodes from {dataset_dir}. "
                "Please verify data_path, scenes_dir, and dataset files."
            )
        ...
        if config.content_scenes and config.content_scenes != ["*"]:
            ...
            if not self.episodes:
                raise ValueError(
                    f"DynamicVLNCEDataset filtered to 0 episodes with content_scenes={config.content_scenes}."
                )
