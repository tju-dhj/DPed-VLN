...
def continuous_vector_action_to_hab_dict_v3(
    ...
    actual_action_space = original_action_space.spaces.copy()
    if 'agent_0_move_forward' in original_action_space.spaces:
        agent_0_action_list = ['agent_0_stop','agent_0_move_forward','agent_0_turn_left','agent_0_turn_right','agent_0_pause','agent_0_move_backward']
        agent_0_action_index = int(action[0])
        selected_agent_0_action = agent_0_action_list[agent_0_action_index]
        # 只保留 selected_agent_0_action，删除其余 agent_0 动作
        for key in list(actual_action_space.keys()):
        ...
        agent_0_action_list = ['agent_0_discrete_stop','agent_0_discrete_move_forward','agent_0_discrete_turn_left','agent_0_discrete_turn_right','agent_0_discrete_pause','agent_0_discrete_move_backward']
        agent_0_action_index = int(action[0])
        selected_agent_0_action = agent_0_action_list[agent_0_action_index]
        # 只保留 selected_agent_0_action，删除其余 agent_0 动作
        for key in list(actual_action_space.keys()):
...
class HabGymWrapper(gym.Wrapper):
    ...
    def step(
        ...
        if isinstance(self.action_space, spaces.Box):
            ...
            # For Discrete action spaces, Habitat accepts either an action name
            # (str) or an action index (int). When the task action space is a
            # Mapping (ActionSpace), the index->name mapping depends on the
            # insertion order of task actions. We keep Habitat's mapping but
            # expose it for debugging to avoid silent mismatches when extending
            # action sets (e.g., adding pause/backward).
            if hasattr(self, "original_action_space") and hasattr(
                self.original_action_space, "spaces"
            ):
                if not hasattr(self, "_printed_discrete_action_mapping"):
                    self._printed_discrete_action_mapping = False
                if not self._printed_discrete_action_mapping:
                    try:
                        keys = list(self.original_action_space.spaces.keys())
                    except Exception:
                        keys = None
                    print(
                        "[HabGymWrapper] Discrete action mapping (index->key): ",
                        keys,
                        "(n=", len(keys) if keys is not None else "?", ")",
                        flush=True,
                    )
                    self._printed_discrete_action_mapping = True
