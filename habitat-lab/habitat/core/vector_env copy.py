...
class VectorEnv:
    ...
    def current_episodes(self, all_info: bool = True):
        """Get current episodes from all environments.
        
        Args:
            all_info: If True (default), returns full episode info including gt_action.
                     If False, returns only episode_id and scene_id.
        """
        for write_fn in self._connection_write_fns:
            # 传递 all_info 参数给 current_episode() 方法
            write_fn((CALL_COMMAND, (CURRENT_EPISODE_NAME, {"all_info": all_info})))
        results = []
