...
class SpotRobot(MobileManipulator):
    def _get_spot_params(self):
        return MobileManipulatorParams(
            ...
            cameras={
                 "overhead_front_rgb": ArticulatedAgentCameraParams(
                    cam_offset_pos=mn.Vector3(0.166, 0.83, 0.0),
                    cam_orientation=mn.Vector3(0, -1.571, 0.0),
                    attached_link_id=-1,
                ),
                "overhead_front_depth": ArticulatedAgentCameraParams(
                cam_offset_pos=mn.Vector3(0.166, 0.83, 0.0),      # 与RGB相机相同位置
                cam_orientation=mn.Vector3(0, -1.571, 0.0),      # 向下
                attached_link_id=-1,                           # 连接到基座
               ),
                ...
                "articulated_agent_jaw_depth": ArticulatedAgentCameraParams(
                    cam_offset_pos=mn.Vector3(0.166, 1.0, -0.107),
                    cam_orientation=mn.Vector3(0, -1.571, 0.0),
                    attached_link_id=6,
                    relative_transform=mn.Matrix4.rotation_z(mn.Deg(-90)),
                ),
