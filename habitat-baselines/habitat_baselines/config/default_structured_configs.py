...
class EvalConfig(HabitatBaselinesBaseConfig):
    ...
    # Optional upper bound on steps per episode (-1: no cap)
...
class PolicyConfig(HabitatBaselinesBaseConfig):
    ...
    turn_step: int = 15
    #streamvln
    num_frames: int = 32    # 帧数，在重置内存之前处理的帧数
...
class PPOConfig(HabitatBaselinesBaseConfig):
    ...
    # GRPO-style grouped advantage settings for environment RL.
    grpo_group_size: int = 4
    grpo_min_group_size: int = 2
    grpo_group_key: str = "instruction"
    grpo_reward_aggregation: str = "sum"
    grpo_advantage_eps: float = 1e-5
    grpo_normalize_std: bool = True
    grpo_fallback_to_gae: bool = True
    grpo_use_batch_fallback: bool = True
    grpo_use_value_critic: bool = False
    grpo_ref_kl_coef: float = 0.0
    grpo_advantage_clip: float = 5.0
    grpo_global_norm_weight: float = 0.3
    # KL annealing: linearly anneal ref_kl_coef from anneal_start to anneal_end
    # over anneal_steps updates. If unset, no annealing (use ref_kl_coef directly).
    grpo_ref_kl_anneal_start: Optional[float] = None
    grpo_ref_kl_anneal_end: Optional[float] = None
    grpo_ref_kl_anneal_steps: int = 0
    # Adaptive global-norm-weight: reduce global mixing when groups are large.
    grpo_adaptive_global_norm: bool = True
    grpo_global_norm_weight_min: float = 0.05
    # Entropy anti-collapse: warn/freeze if policy entropy drops below threshold.
    grpo_entropy_min_threshold: float = 0.01
    grpo_entropy_collapse_action: str = "warn"  # "warn" or "skip_update"
    # Small-group advantage scaling: down-weight advantages from tiny groups.
    grpo_scale_adv_by_group_size: bool = True
...
@dataclass
class GNNConfig(HabitatBaselinesBaseConfig):
    """Spatio-Temporal Graph Neural Network config"""
...
@dataclass
class DetectionConfig(HabitatBaselinesBaseConfig):
...
@dataclass
class FusionConfig(HabitatBaselinesBaseConfig):
...
class DDPPOConfig(HabitatBaselinesBaseConfig):
    ...
    # 文本指令处理配置
    use_text_instruction: bool = True
    instruction_data_path: Optional[str] = None
    instruction_matching_mode: str = "episode_based"
    text_encoder_dim: int = 512
    fusion_method: str = "attbpool"
    
    # CLIP模型类型: "longclip" (默认，支持248 tokens长文本) 或 "clip" (标准77 tokens)
    clip_model_type: str = "longclip"

    # CLIP视觉传感器配置（多视角支持）
    clip_visual_sensors: Optional[Dict[str, Any]] = None

    # GPS传感器类型: "starting_point"(起始点GPS) 或 "pointgoal"(目标点GPS) 或 "original"(原始传感器)
    gps_sensor_type: str = "starting_point"
...
class RLConfig(HabitatBaselinesBaseConfig):
    ...
    # ST-GNN (Spatio-Temporal Graph Neural Network) config
...
class HabitatBaselinesConfig(HabitatBaselinesBaseConfig):
    ...
    # If True, resume loads model weights but skips optimizer/lr-scheduler state.
    ...
    # Brain configuration for instruction optimization
    brain: Optional[Dict[str, Any]] = None
    # Load from pretrained checkpoint (not resume state)
    load_from_checkpoint: bool = False
    checkpoint_path: Optional[str] = None
    # Load actor weights from an IL checkpoint before RL/GRPO training starts.
    load_from_il_checkpoint: bool = False
