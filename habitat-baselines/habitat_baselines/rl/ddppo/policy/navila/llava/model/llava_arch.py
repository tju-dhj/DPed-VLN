...
class LlavaMetaForCausalLM(ABC):
    ...
    def generate(
        ...
        **generation_kwargs,
        ...
        # 修复 transformers 4.56.0 兼容性问题：
        # LlamaForCausalLM.generate() 不支持直接传递 inputs_embeds 参数
        # 需要通过 prepare_inputs_for_generation 来支持 inputs_embeds
        # 方法：将 inputs_embeds 和 attention_mask 放入 model_kwargs 中
        # 注意：不能同时传递 input_ids 和 inputs_embeds，所以设置 input_ids=None
        
        # 确保所有输入都在模型所在的设备上（避免设备不匹配错误）
        logger = logging.getLogger(__name__)
        
        try:
            llm_device = next(self.llm.parameters()).device
        except (StopIteration, AttributeError):
            # 如果无法获取设备，尝试使用模型的 device 属性
            llm_device = getattr(self.llm, 'device', None)
            if llm_device is None:
                # 如果还是没有，使用 inputs_embeds 的设备
                llm_device = inputs_embeds.device
        
        # 检查 LLM 的所有参数设备
        try:
            llm_param_devices = set()
            device_count = {}
            for name, param in self.llm.named_parameters():
                dev_str = str(param.device)
                llm_param_devices.add(dev_str)
                device_count[dev_str] = device_count.get(dev_str, 0) + 1
            
            # 如果发现多个设备，需要将所有参数移动到同一设备
            if len(llm_param_devices) > 1:
                logger.warning(f"  WARNING: LLM parameters are distributed across multiple devices: {llm_param_devices}")
                logger.warning(f"  This will cause device mismatch errors. Attempting to move all to {llm_device}...")
                try:
                    # 尝试将整个模型移动到单个设备
                    ...
                    new_devices = set()
                    for name, param in self.llm.named_parameters():
                        new_devices.add(str(param.device))
                    if len(new_devices) > 1:
                        logger.error(f"  ERROR: Failed to consolidate LLM to single device. Still on: {new_devices}")
                except Exception as e:
                    logger.error(f"  ERROR: Failed to move LLM to single device: {e}")
        except Exception as e:
            logger.warning(f"  Could not check LLM parameter devices: {e}")
        
        # 确保 inputs_embeds 和 attention_mask 在正确的设备上
        # 如果 LLM 分布在多个设备上，我们需要确保输入在第一个设备上
        # 但实际上，如果模型分散了，我们需要强制移动到单个设备
        if inputs_embeds.device != llm_device:
            inputs_embeds = inputs_embeds.to(llm_device)
            
        if attention_mask is not None:
            if attention_mask.device != llm_device:
                attention_mask = attention_mask.to(llm_device)
        
        # 再次检查 LLM 设备，确保在调用 generate 之前所有参数都在同一设备
        try:
            final_devices = set()
            for name, param in self.llm.named_parameters():
                final_devices.add(str(param.device))
            if len(final_devices) > 1:
                logger.error(f"  CRITICAL: LLM still on multiple devices before generate: {final_devices}")
                logger.error(f"  This will cause device mismatch. Attempting emergency fix...")
                # 紧急修复：强制移动到第一个设备
                target_device = list(final_devices)[0]
                logger.warning(f"  Moving all LLM parameters to {target_device}")
                
                # 尝试禁用 accelerate hooks（如果存在）
                # 注意：不能设置为 None，因为 transformers 代码会访问 .values()
                # 应该设置为空字典 {}，表示没有设备映射
                if hasattr(self.llm, 'hf_device_map'):
                    logger.warning(f"  Disabling hf_device_map: {self.llm.hf_device_map}")
                    self.llm.hf_device_map = {}
                
                # 移动模型
                self.llm = self.llm.to(target_device)
                
                # 如果仍然分散，尝试逐个移动参数
                new_final_devices = set()
                for name, param in self.llm.named_parameters():
                    new_final_devices.add(str(param.device))
                if len(new_final_devices) > 1:
                    logger.warning(f"  Model.to() failed, moving parameters individually...")
                    for name, param in self.llm.named_parameters():
            ...
            logger.warning(f"  Could not verify LLM device before generate: {e}")
        
        model_kwargs = generation_kwargs.pop('model_kwargs', {})
        model_kwargs['inputs_embeds'] = inputs_embeds
        if attention_mask is not None:
            model_kwargs['attention_mask'] = attention_mask
        
        # 传递 input_ids=None，因为我们使用 inputs_embeds
        # generation_kwargs 中的其他参数（如 max_new_tokens, use_cache 等）会正常传递
        outputs = self.llm.generate(
            input_ids=None,  # 必须为 None，因为使用 inputs_embeds
            **generation_kwargs,
            **model_kwargs
        )
        return outputs
