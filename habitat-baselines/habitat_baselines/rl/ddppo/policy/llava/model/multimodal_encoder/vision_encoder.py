...
try:
    ...
    def multiscale_forward(forward_fn, *args, **kwargs):
        warnings.warn(
            "s2wrapper 未安装，回退到单尺度特征提取，这可能降低多尺度感知效果。",
            RuntimeWarning,
        )
...
class VisionTower(nn.Module):
    ...
    def _maybe_resize_pos_embeds(
        ...
        if interpolate_mode == "linear":
            ...
            raise NotImplementedError(f"Unsupported interpolate_mode: {interpolate_mode}")
