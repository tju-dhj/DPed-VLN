...
def resume_state_filename(config: DictConfig, filename_key: str = "") -> str:
    ...
    if is_slurm_job():
        # 使用OmegaConf的安全访问方式检查RL配置
        from omegaconf import OmegaConf
        if OmegaConf.is_config(config.habitat_baselines) and 'rl' in config.habitat_baselines:
            rl_config = config.habitat_baselines.rl
            if OmegaConf.is_config(rl_config) and 'preemption' in rl_config:
                preemption_config = rl_config.preemption
                if OmegaConf.is_config(preemption_config):
                    append_slurm_job_id = preemption_config.get('append_slurm_job_id', False)
        # IL配置通常不需要append_slurm_job_id，因为IL训练通常不使用SLURM preemption
    
    if append_slurm_job_id:
...
def init_distrib_slurm(
    ...
    # ========== NCCL 异步错误处理 & 超时配置 ==========
    # 1. TORCH_NCCL_ASYNC_ERROR_HANDLING: 启用异步 watchdog 检测超时/错误
    #    替代已废弃的 NCCL_ASYNC_ERROR_HANDLING，无性能开销
    if "TORCH_NCCL_ASYNC_ERROR_HANDLING" not in os.environ:
        os.environ["TORCH_NCCL_ASYNC_ERROR_HANDLING"] = "1"

    # 2. TORCH_NCCL_BLOCKING_WAIT: 与 ASYNC_ERROR_HANDLING 互斥，确保不一致
    #    ASYNC_ERROR_HANDLING=1 时 BLOCKING_WAIT 必须为 0
    if "TORCH_NCCL_BLOCKING_WAIT" not in os.environ:
        os.environ["TORCH_NCCL_BLOCKING_WAIT"] = "0"

    # 3. TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC: watchdog 心跳超时（秒）
    #    超过此时间未收到其他 rank 的心跳则认为死锁
    if "TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC" not in os.environ:
        os.environ["TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC"] = "600"

    # 4. NCCL 调试日志（生产环境可注释掉）
    if "NCCL_DEBUG" not in os.environ:
        os.environ["NCCL_DEBUG"] = "WARN"  # 出问题时改为 INFO 获取详细日志
    ...
    # 设置 NCCL 超时时间为 30 分钟（默认 10 分钟对于 VLM+Habitat 场景不够）
