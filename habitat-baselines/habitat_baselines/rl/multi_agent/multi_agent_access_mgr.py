...
class MultiAgentAccessMgr(AgentAccessMgr):
    ...
    def load_state_dict(self, state, strict: bool = False):
        # Check if state is a flat dictionary (IL checkpoint format)
        # Flat dictionaries have model parameter names as keys (e.g., 'net.prev_action_embedding.weight')
        # rather than agent IDs (e.g., '0', '1')
        state_keys = list(state.keys())
        
        # Determine if this is a flat dictionary:
        # - If keys are model parameter names (contain '.' or '_' and are not just digits)
        # - If no keys are simple integers or digit strings that could be agent IDs
        is_flat_dict = False
        if len(state_keys) > 0:
            # Check if keys look like model parameter names (contain dots, underscores, or are not simple digits)
            has_model_param_keys = any(
                ('.' in str(k) or '_' in str(k) or not str(k).isdigit()) 
                for k in state_keys[:10]  # Check first 10 keys as sample
            )
            # Check if no keys are simple agent IDs (pure integers or digit strings)
            has_agent_id_keys = any(
                (isinstance(k, int) and k >= 0) or (isinstance(k, str) and k.isdigit() and int(k) >= 0)
                for k in state_keys
            )
            # If it has model param keys and no agent ID keys, it's a flat dict
            is_flat_dict = has_model_param_keys and not has_agent_id_keys
        
        # Get list of agents that need to load state
        agents_to_load = [
            (agent_i, agent) 
        ...
        # If it's a flat dictionary and we have exactly one agent to load, load directly
        if is_flat_dict and len(agents_to_load) == 1:
            agent_i, agent = agents_to_load[0]
            return agent.load_state_dict(state, strict=strict)
        
        last_missing_keys = []
        last_unexpected_keys = []
        # Otherwise, use the multi-agent format (state[agent_i] or state[str(agent_i)])
        for agent_i, agent in agents_to_load:
            # Attempt to load state using the integer key first.
            try:
                last_missing_keys, last_unexpected_keys = agent.load_state_dict(
                    state[agent_i], strict=strict
                )
            except KeyError:
                # If the integer key is not found, try using the string representation of the integer.
                try:
                    last_missing_keys, last_unexpected_keys = agent.load_state_dict(
                        state[str(agent_i)], strict=strict
                    )
        ...
        return last_missing_keys, last_unexpected_keys
