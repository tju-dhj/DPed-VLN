...
class DPedProRewardCalculator:
    ...
    def __init__(self, config: Union[Dict, Any]):
        ...
        # ── 距离奖励开关 ──
        # 【关键修复】默认关闭，避免与 MultiAgentNavReward 中的距离奖励叠加
    ...
    def compute_reward(
        ...
        # ── 1. 基础距离奖励 ──
        distance_reward = 0.0
        if self.use_distance_reward and prev_distance_to_goal is not None and current_distance_to_goal is not None:
        ...
        if collision_count > 0:
            scaled_penalty = self.collision_penalty_val * collision_count * self.collision_penalty_scale
            total_reward += scaled_penalty
            self.stats["collision_penalty"] += scaled_penalty
            extra_info["collision_penalty"] = scaled_penalty
    ...
    def _compute_safe_distance_reward(self, observations: Dict) -> float:
        """与行人保持安全距离时给予正奖励，鼓励主动避让"""
        reward = 0.0
        if not isinstance(observations, dict):
            return reward
        robot_pos = observations.get("agent_0_localization_sensor", None)
        if robot_pos is None or len(robot_pos) < 3:
            return reward
        robot_pos = np.array(robot_pos[:3])
        for i in range(1, 7):
            human_pos = observations.get(f"agent_{i}_localization_sensor", None)
            if human_pos is None or len(human_pos) < 3:
                continue
            distance = np.linalg.norm(np.array(human_pos[:3]) - robot_pos)
