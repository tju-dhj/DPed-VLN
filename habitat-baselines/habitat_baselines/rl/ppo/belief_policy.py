    ...
    def __init__(
        ...
        **kwargs,  # Note, we forward kwargs to the net
        ...
        super().__init__(net(
            ...
            **kwargs,
        ), action_space, policy_config=config.habitat_baselines.rl.policy.get(kwargs["agent_name"],"main_agent"))
        self.aux_tasks = aux_tasks

        resnet_baseplanes = 32
        backbone = config.habitat_baselines.rl.ddppo.backbone # "resnet18"
        ...
        visual_resnet = resnet_policy.ResNetEncoder(
            ...
            normalize_visual_inputs=False # config.use_mean_and_var
        )
...
class MultipleBeliefPolicy(BeliefPolicy):
    ...
    def evaluate_actions(
        self, observations, rnn_hidden_states, prev_actions, masks, action, rnn_build_seq_info,
    ):
        visual_embedding = self.visual_encoder(observations)
        # sequence forwarding
        features, rnn_hidden_states, individual_features, weights = self.net(
            visual_embedding, observations, rnn_hidden_states, prev_actions,
            masks,rnn_build_seq_info
        )
...
class AttentiveBelief(MultipleBeliefNet):
    ...
    def forward(self, visual_embedding, observations, rnn_hidden_states,
        ...
        # rnn_hidden_states.size(): num_layers, num_envs, num_tasks, hidden, (only first timestep)
        if len(args) > 0:
            outputs = [encoder(x, rnn_hidden_states[:, :, i], masks, args[0]) for i, encoder
                    in enumerate(self.state_encoders)]
        else:
            outputs = [encoder(x, rnn_hidden_states[:, :, i], masks) for i, encoder
                    in enumerate(self.state_encoders)]           
...
class AttentiveBeliefPolicy(MultipleBeliefPolicy):
    ...
    def from_config(cls, config, observation_space, action_space, **kwargs):
        ...
        return cls(observation_space, action_space, config.habitat_baselines.rl.ppo.hidden_size, 
                   AttentiveBelief, 
                   aux_tasks=kwargs.get("aux_tasks",[]),
                   goal_sensor_uuid=config.habitat.task.goal_sensor_uuid,
                   num_tasks=len(config.habitat_baselines.rl.AUX_TASKS.tasks),
                   config=config, # .habitat_baselines.rl.policy.get(agent_name,"main_agent") , # main_agent
                   orig_action_space=kwargs["orig_action_space"],
