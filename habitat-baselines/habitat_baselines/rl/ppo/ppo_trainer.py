...
class PPOTrainer(BaseRLTrainer):
    ...
    def save_checkpoint(
        ...
        full_sd = checkpoint[0]["state_dict"]
        future_trajectory_aux_state = {
            k[len("aux_loss_modules."):] : v
            for k, v in full_sd.items()
            if k.startswith("aux_loss_modules.future_trajectory_prediction.")
        }
        # Optionally save auxiliary heads separately
        print("should_save_aux", getattr(self.config, "should_save_aux", False))

        if getattr(self.config, "should_save_aux", False):
            full_sd = checkpoint[0]["state_dict"]
            future_trajectory_aux_state = {
                k[len("aux_loss_modules."):] : v
                for k, v in full_sd.items()
            ...
            print("aux_path", aux_path)
            if future_trajectory_aux_state:
                torch.save({"aux_state_dict": future_trajectory_aux_state}, aux_path)
                print("aux_state saved")
    ...
    def _eval_checkpoint(
        ...
        if self.config.habitat_baselines.eval.should_load_ckpt:
            ...
            # Handle both RL checkpoints (with extra_state) and IL checkpoints (without extra_state)
            if "extra_state" in ckpt_dict and "step" in ckpt_dict["extra_state"]:
                step_id = ckpt_dict["extra_state"]["step"]
                logger.info(f"Loaded checkpoint trained for {step_id} steps")
            else:
                step_id = checkpoint_index
                logger.info(f"Loaded IL checkpoint (no step info available), using checkpoint index {step_id}")
        else:
        ...
        if (
            ...
            # Check if this is DDPPO format (has integer keys like 0, 1, 2...)
            has_int_keys = any(isinstance(k, int) for k in ckpt_dict.keys())
            
            if has_int_keys:
                # DDPPO format: {0: {state_dict: ...}, 1: {...}, ...}
                self._agent.load_state_dict(ckpt_dict)
            else:
                # DAgger format: flat weights {net.xxx: tensor, ...} or {net.xxx: tensor, ..., 'config': ...}
                # Extract only model weights (exclude metadata like 'config')
                model_weights = {k: v for k, v in ckpt_dict.items() 
                               if not isinstance(k, str) or (k != 'config' and not k.startswith('extra_state'))}
                # Convert to expected format for multi-agent manager: {0: {state_dict: ...}}
                converted_dict = {0: {"state_dict": model_weights}}
                self._agent.load_state_dict(converted_dict)

        # Optionally load auxiliary heads from separate checkpoint for evaluation
      
        if getattr(self.config, "should_load_aux", False):
            aux_path = getattr(self.config, "aux_ckpt_path", None)
            if aux_path is None or not os.path.isfile(aux_path):
                logger.warn(
                    f"Aux checkpoint path not found or not set: {aux_path}"
                )
            else:
                aux_ckpt = torch.load(aux_path, map_location="cpu", weights_only=False)
                aux_state = aux_ckpt.get("aux_state_dict", aux_ckpt)
                if hasattr(self._agent.actor_critic, "aux_loss_modules"):
                    # Strict=False to be robust to partial aux selection
                    self._agent.actor_critic.aux_loss_modules.load_state_dict(
                        aux_state, strict=False
                    )
                    if self.config.habitat_baselines.verbose:
                        logger.info(
                            f"Loaded auxiliary modules from {aux_path}"
                        )
