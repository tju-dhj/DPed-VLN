...
def _save_eval_resume(config, stats_episodes, ep_eval_count) -> None:
    ...
    def _safe_convert(val):
        """Convert numpy/unsupported types to JSON-serializable types."""
    ...
    for (ep_key, eval_count), v in stats_episodes.items():
        ...
        serializable_stats[key] = {
            k: _safe_convert(vv) for k, vv in v.items()
        }
    ...
    try:
        with open(abs_resume_path, "w") as f:
...
class NaVILAEvaluator(FALCONEvaluator):
    ...
    def evaluate_agent(
        ...
        if total_completed > 0:
            ...
            total_act_dim = int(np.prod(action_shape))
            skipped = 0
            ff_errors = 0
            while skipped < total_completed:
                try:
                    stop_actions = np.zeros(
                        (envs.num_envs, total_act_dim), dtype=np.float32
                    )
                    outputs = envs.step(
                    ...
                    ff_errors += 1
                    logger.error(
                        f"[eval resume] Fast-forward step failed (error {ff_errors}): {e}. "
                        f"Attempting to continue from episode {skipped + 1}..."
                    )
                    if ff_errors > 10:
                        logger.error(
                            f"[eval resume] Too many fast-forward errors ({ff_errors}). "
                            f"Giving up fast-forward. Will start evaluation from current episode."
                        )
                        break
            if ff_errors > 0:
                logger.warning(
                    f"[eval resume] Fast-forward completed with {ff_errors} errors. "
                    f"Some episodes may have been skipped incorrectly."
                )
            observations = envs.post_step(observations)
            batch = batch_obs(observations, device=device)
            batch = apply_obs_transforms_batch(batch, obs_transforms)
            # Seed the RGB history for the new episode
            rgb_key = _find_rgb_key(batch, config)
            if rgb_key is not None:
                for env_i in range(envs.num_envs):
                    past_rgbs[env_i] = [
                        Image.fromarray(
                            np.uint8(batch[rgb_key][env_i].cpu().numpy())
                        ).convert("RGB")
                    ]
            logger.info(
        ...
        while (
            ...
            # 执行动作（包裹在try-except中以处理场景加载失败等Python异常）
            try:
                outputs = envs.step(step_data)
                observations, rewards_l, dones, infos = [list(x) for x in zip(*outputs)]
            except Exception as step_exc:
                logger.error(
                    f"[eval] envs.step() failed at episode "
                    f"scene={current_episodes_info[0].scene_id if envs.num_envs > 0 else '?'} "
                    f"ep={current_episodes_info[0].episode_id if envs.num_envs > 0 else '?'}: "
                    f"{step_exc}"
                )
                logger.error(f"[eval] Traceback: {traceback.format_exc()}")
                # Mark this episode as failed and save progress before re-raising
                for i in range(envs.num_envs):
                    if current_episodes_info[i] is not None:
                        ep_k = (current_episodes_info[i].scene_id, current_episodes_info[i].episode_id)
                        ep_eval_count[ep_k] = evals_per_ep  # Mark as done (failed)
                        stats_episodes[(ep_k, evals_per_ep)] = {
                            "reward": 0.0, "success": 0.0, "spl": 0.0,
                            "psc": 0.0, "distance_to_goal": 0.0,
                            "num_steps": episode_steps[i],
                            "_eval_error": str(step_exc)[:200],
                        }
                        pbar.update()
                _save_eval_resume(config, stats_episodes, ep_eval_count)
                raise  # Re-raise - let the outer handler or process decide
