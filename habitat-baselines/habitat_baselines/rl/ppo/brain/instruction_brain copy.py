...
class InstructionBrain:
    ...
    def _optimize_with_api(self, original_instruction: str, current_frame: np.ndarray, history_frames: Optional[List[FrameRecord]],
        ...
        # 增强的动作约束说明（6个动作，动作轨迹示例改为5个动作匹配5帧历史）
        context_text = (
            f"=== NAVIGATION TASK ===\n"
            ...
            f"  - 4 (WAIT): Robot pauses in place for a moment, then can continue with next action\n"
            f"  - 5 (BACKWARD): Robot moves backward 0.25 meters\n"
            f"\n=== WHEN TO USE WAIT (Action 4) ===\n"
            f"Use WAIT when:\n"
            f"  1. A pedestrian is moving toward you and you need time to assess their trajectory\n"
            f"  2. The pedestrian is in an unpredictable path and stopping gives them space to pass\n"
            f"  3. You want the robot to pause and let a pedestrian cross safely\n"
            f"  4. Immediate movement would cause a collision, so you wait for the pedestrian to clear\n"
            f"\n=== TRAJECTORY FORMAT (5 Actions for 5-Frame History) ===\n"
            f"Example: 'forward 0.25m ×1 → turn right 15° ×1 → forward 0.25m ×1 → wait ×1 → stop ×1'\n"
            f"This means: move forward 0.25m, rotate right 15°, move forward 0.25m, pause briefly, then stop.\n"
    ...
    def _build_full_prompt_text(
        ...
        # 动作约束说明（6个动作，动作轨迹示例改为5个动作匹配5帧历史）
