...
DatasetBase: Type = getattr(wds, "Dataset", None)
if DatasetBase is None:
    DatasetBase = getattr(wds, "WebDataset", None)
if DatasetBase is None:
    raise ImportError(
        "webdataset package must provide either Dataset or WebDataset class; "
        "please verify the installed webdataset version."
    )


class NavDataset(DatasetBase):
