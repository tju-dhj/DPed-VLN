...
def test_sensor_registration():
    ...
    # Check if config sensors are registered
    try:
        instruction_config = registry.get_sensor("instruction_sensor")
        print("✅ instruction_sensor config registered")
    except KeyError:
        print("❌ instruction_sensor config NOT registered")
    
    try:
        gt_action_config = registry.get_sensor("gt_action_sensor")
        print("✅ gt_action_sensor config registered")
    except KeyError:
        print("❌ gt_action_sensor config NOT registered")
