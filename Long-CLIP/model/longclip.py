...
try:
    ...
    _is_pkg_resources_packaging = True
except ImportError:
    import packaging.version as _packaging_version
    _is_pkg_resources_packaging = False

def _version_parse(v):
    """Parse version string, compatible with both pkg_resources and packaging."""
    if _is_pkg_resources_packaging:
        return packaging.version.parse(v)
    else:
        return _packaging_version.parse(v)
...
if _version_parse(torch.__version__) < _version_parse("1.7.1"):
...
def tokenize(texts: Union[str, List[str]], context_length: int = 77*4-60, truncate: bool = False) -> Union[torch.IntTensor, torch.LongTensor]:
    ...
    if _version_parse(torch.__version__) < _version_parse("1.8.0"):
